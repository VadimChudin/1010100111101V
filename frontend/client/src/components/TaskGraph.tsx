// The canvas is a Git-derived project map; agent plans remain a fallback before the workspace loads.
...
import { Check, CircleDashed, FileCode2, Files, FolderTree, GitBranch, Loader2, RefreshCw, Wrench } from 'lucide-react'
...
function ModuleNode({ data }: NodeProps) {
  ...
  return <div className={`min-w-[215px] rounded-[16px] border p-4 shadow-xl transition-colors ${selected ? 'border-signal-ice/80 bg-signal-ice/[0.12]' : 'border-white/15 bg-[#101820]/95 hover:border-signal-ice/40'}`}><Handle type="target" position={Position.Left} className="!h-2 !w-2 !border-0 !bg-signal-ice" /><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-signal-ice">{module.kind} · {module.origin}</p><h3 className="mt-1 font-display text-[14px] font-semibold text-white">{module.title}</h3></div><span className="rounded border border-white/10 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.1em] text-text-dim">{module.status}</span></div><p className="mt-2 truncate font-mono text-[9px] text-text-dim">{module.source_scope || 'No source scope'}</p>{Object.keys(counts).length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{Object.entries(counts).map(([type, count]) => <span key={type} className={`rounded-full px-2 py-0.5 font-mono text-[9px] ${type === 'error' || type === 'blocked' ? 'bg-alert/15 text-alert' : type === 'task' ? 'bg-signal-ice/15 text-signal-ice' : 'bg-white/[0.07] text-text-muted'}`}>{type} {count}</span>)}</div>}<Handle type="source" position={Position.Right} className="!h-2 !w-2 !border-0 !bg-signal-ice" /></div>
}

function RepositoryTree({ files, selectedModule }: { files: RepositoryFile[]; selectedModule?: WorkspaceModule }) {
  const sourceFiles = useMemo(() => files.filter((file) => file.kind === 'file' && (!selectedModule?.source_scope || file.path.startsWith(selectedModule.source_scope))).slice(0, 8), [files, selectedModule?.source_scope])
  return <aside className="absolute bottom-5 right-5 z-10 hidden w-[278px] rounded-xl border border-white/10 bg-[#0c1117]/90 p-3 shadow-2xl backdrop-blur xl:block"><div className="flex items-center justify-between"><div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.13em] text-signal-ice"><FolderTree size={13} /> Git file tree</div><span className="font-mono text-[9px] text-text-dim">{files.filter((file) => file.kind === 'file').length} files</span></div>{selectedModule && <p className="mt-2 truncate font-mono text-[9px] text-text-dim">{selectedModule.source_scope}</p>}<div className="mt-3 max-h-[205px] space-y-1 overflow-y-auto">{sourceFiles.length ? sourceFiles.map((file) => <div key={file.path} title={file.path} className="flex items-center gap-2 rounded px-1.5 py-1 text-[10px] text-text-muted"><FileCode2 size={12} className="shrink-0 text-signal-ice/80" /><span className="min-w-0 truncate font-mono">{file.path}</span><span className="ml-auto shrink-0 font-mono text-[8px] text-text-dim">{file.language || 'file'}</span></div>) : <p className="px-1.5 py-3 text-xs leading-5 text-text-dim">Index the repository to expose its tracked file tree.</p>}</div></aside>
}
...
export default function TaskGraph({ plan, events, connected, workspace, repository, files, indexing = false, selectedModuleId, onSelectModule, onIndexRepository }: { plan: PlanStep[]; events: AgentEvent[]; connected: boolean; workspace?: WorkspaceSnapshot; repository?: RepositoryIndex; files: RepositoryFile[]; indexing?: boolean; selectedModuleId?: string; onSelectModule?: (moduleId: string) => void; onIndexRepository?: () => void }) {
  ...
  const repositoryLabel = repository ? `${repository.branch} · ${repository.commit_sha.slice(0, 8)}` : 'Not indexed'
  return <section className="relative min-h-[620px] flex-1 overflow-hidden bg-[#0a0e13]" aria-label="Project workspace graph"><div className="absolute left-5 top-5 z-10 sm:left-8 sm:top-7"><p className="eyebrow">{usingWorkspace ? 'Git project map' : 'Execution topology'}</p><h2 className="mt-1 font-display text-xl font-semibold tracking-[-0.03em] text-white">{usingWorkspace ? workspace!.project.name : 'Task graph'}</h2><p className="mt-1 max-w-[360px] text-xs leading-5 text-text-dim">{usingWorkspace ? selected ? `${selected.title} is in focus. The graph comes from tracked source scopes; notes and tasks remain durable agent context.` : 'Select a Git-derived module to inspect its files and durable context.' : 'A live map of the agent plan and the work moving through it.'}</p><div className="mt-3 flex flex-wrap items-center gap-2"><span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-[#101820]/80 px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-text-dim"><GitBranch size={11} className="text-signal-ice" /> {repositoryLabel}</span>{repository && <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-[#101820]/80 px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-text-dim"><Files size={11} className="text-signal-ice" /> {repository.files_count} tracked</span>}<button onClick={onIndexRepository} disabled={indexing || !onIndexRepository} className="flex items-center gap-1.5 rounded-full border border-signal-ice/30 bg-signal-ice/[0.07] px-2.5 py-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-signal-ice transition-colors hover:bg-signal-ice/[0.14] disabled:cursor-not-allowed disabled:opacity-60"><RefreshCw size={11} className={indexing ? 'animate-spin' : ''} /> {indexing ? 'Indexing' : 'Refresh map'}</button></div></div><div className="absolute right-5 top-5 z-10 flex items-center gap-2 rounded-full border border-white/10 bg-[#101820]/80 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-text-dim backdrop-blur sm:right-8 sm:top-7"><span className={`h-1.5 w-1.5 rounded-full ${connected ? 'bg-complete live-dot' : 'bg-alert'}`} />{connected ? 'Stream synced' : 'Stream offline'}</div><div className="absolute bottom-5 left-5 z-10 max-w-[250px] font-mono text-[10px] uppercase tracking-[0.1em] text-text-dim sm:bottom-7 sm:left-8">{usingWorkspace ? `${workspace!.modules.length} Git modules · ${workspace!.markers.length} context markers` : plan.length ? `${plan.length} steps in current plan` : 'No active plan'}</div><RepositoryTree files={files} selectedModule={selected} /><ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(_, node) => usingWorkspace && onSelectModule?.(node.id)} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: 0.35 }} minZoom={0.45} maxZoom={1.35} proOptions={{ hideAttribution: true }}><Background color="#29343e" gap={28} size={1} /><Controls showInteractive={false} /><MiniMap nodeColor={(node) => node.type === 'module' ? '#527487' : ((node.data as { step?: PlanStep })?.step?.status === 'active' ? '#9de8ff' : '#344653')} maskColor="rgba(6, 10, 14, 0.76)" /></ReactFlow></section>
}
