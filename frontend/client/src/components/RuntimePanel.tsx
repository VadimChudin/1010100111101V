import { BrainCircuit, Check, Clock3, Laptop, Link2, ShieldCheck, X } from 'lucide-react'
import type { DeviceJob, GraphitiEpisodeEnvelope, ProjectDevice } from '../types'
...
type RuntimePanelProps = {
  ...
  onQueueIndex: (deviceId: string) => Promise<unknown>
...
export default function RuntimePanel({ devices, episodes, jobs, pairing, onPair, onQueueIndex, onApproveJob }: RuntimePanelProps) {
  ...
  return <aside className="absolute right-4 top-4 z-20 hidden w-[296px] border border-white/[0.10] bg-[#0b1017]/95 p-3 shadow-2xl backdrop-blur-xl xl:block">
    ...
    <div className="mt-3 space-y-2">
      {devices.length === 0 ? <p className="rounded border border-dashed border-white/10 p-2 font-mono text-[9px] leading-relaxed text-text-dim">No PC runtime paired. Cloud state remains available; pair a runtime to enable local Serena and Graphiti.</p> : devices.map((device) => <div key={device.id} className="rounded border border-white/[0.08] bg-black/20 p-2">
        ...
        <button disabled={device.status !== 'online'} onClick={() => void onQueueIndex(device.id)} className="mt-2 flex w-full items-center justify-center gap-1 border border-white/[0.10] bg-white/[0.03] py-1.5 font-mono text-[8px] uppercase tracking-[0.10em] text-text-dim transition-colors hover:border-signal-ice/30 hover:text-signal-ice disabled:cursor-not-allowed disabled:opacity-40"><Clock3 size={10} /> Request local index</button>
      </div>)}</div>
